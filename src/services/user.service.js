...
const createUser = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('USUARIO CREADO', 'El usuario se ha creado correctamente','success', 1500, 'top-right');
        return response.data.usuario;
    }
}
...
const updateUser = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('USUARIO MODIFICADO', 'El usuario se ha modificado correctamente','success', 1500, 'top-right');
        return response.data.usuario;
    }
}
...
const deleteUser = async (id, token) => {
    ...
    ToastChakra('USUARIO ELIMINADO', 'El usuario se ha eliminado correctamente','success', 1500, 'top-right');
    return response.data.usuario;
}
