...
const create = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('CERTIFICADO CREADO', 'El certificado se ha agregado correctamente','success', 1500, 'top-right');
...
const update = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('REGISTRO MODIFICADO', 'El certificado se ha modificado correctamente','success', 1500, 'top-right');
        return response.data.certificado;
    }
}
...
const deleteCertificado = async (id, token) => {
    ...
    ToastChakra('CERTIFICADO ELIMINADO', 'El certificado se ha eliminado correctamente','success', 1500, 'top-right');
    return response.data.certificado;
}
