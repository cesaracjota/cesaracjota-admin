...
const create = async (data) => {
    ...
    if (response.status === 200 || response.status === 201) {
        setTimeout(() => {
            ToastChakra('SU MENSAJE FUE ENVIADO', 'El mensaje ha sido registrado correctamente','success', 3500, 'top-right');
        }, 1000)
...
const mensajeLeido = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('EL MENSAJE HA SIDO MARCADO COMO LEIDO', 'El mensaje ha sido marcado leido correctamente','info', 3500, 'top-right');
        return response.data.mensaje;
    }
}
...
const deleteMensaje = async (id, token) => {
    ...
    ToastChakra('MENSAJE ELIMINADO', 'El mensaje se ha eliminado correctamente','success', 1500, 'top-right');
    return response.data.mensaje;
}
