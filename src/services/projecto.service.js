...
const create = async (data, token) => {
    ...
    try {
        ...
        if (response.status === 200 || response.status === 201) {
            ToastChakra('DATA REGISTRADO CREADO', 'El registro se ha agregado correctamente', 'success', 1500, 'top-right');
...
const update = async (data, token) => {
    ...
    if (response.status === 200 || response.status === 201) {
        ToastChakra('REGISTRO MODIFICADO', 'El registro se ha modificado correctamente', 'success', 1500, 'top-right');
        return response.data.project;
    }
}
...
const deleteProject = async (id, token) => {
    ...
    ToastChakra('DATA ELIMINADO', 'El registro se ha eliminado correctamente', 'success', 1500, 'top-right');
    return response.data.project;
}
